module oscillator (
    input  logic        clk_50mhz,
    input  logic        rst,
    input  logic        sample_enable,       // 48 kHz tick
    input  logic [15:0] freq_hz,
    input  logic [1:0]  wave_sel,            // 00=Sine, 01=Square, 10=Sawtooth, 11=Triangle

    // Sine ROM interface
    output logic [11:0]       rom_address,
    input  logic signed [15:0] sine_rom_data,

    output logic signed [15:0] wave_out
);

    // --- 1. Phase Accumulator ---
    logic [31:0] phase_acc;
    logic [31:0] phase_inc;

    // phase_inc = freq_hz * (2^32 / 48000) = freq_hz * 89478
    assign phase_inc = freq_hz * 32'd89478;

    always_ff @(posedge clk_50mhz or posedge rst) begin
        if (rst)
            phase_acc <= '0;
        else if (sample_enable)
            phase_acc <= phase_acc + phase_inc;
    end

    // --- 2. Waveform Generation ---
    logic signed [15:0] sqr_wave;
    logic signed [15:0] saw_wave;
    logic signed [15:0] tri_wave;
    logic        [15:0] tri_unsigned;
    logic        [15:0] tri_centered;

    always_comb begin
        // Square: MSB of phase determines polarity
        sqr_wave = (phase_acc[31] == 1'b0) ? 16'sd16383 : -16'sd16384;

        // Sawtooth: top 16 bits of phase directly
        saw_wave = signed'(phase_acc[31:16]);

        // Triangle: fold the upper half of the sawtooth
        tri_unsigned = phase_acc[30:15] ^ {16{phase_acc[31]}};
        tri_centered = tri_unsigned - 16'h8000;
        tri_wave     = signed'(tri_centered);

        // Sine: address the ROM with top 12 bits of phase
        rom_address = phase_acc[31:20];
    end

    // --- 3. Waveform Multiplexer ---
    // FIX: removed 'unique' keyword; 'unique case' with a 'default' branch is
    //      contradictory (unique asserts all cases are covered without default).
    always_comb begin
        case (wave_sel)
            2'b00:   wave_out = sine_rom_data;
            2'b01:   wave_out = sqr_wave;
            2'b10:   wave_out = saw_wave;
            2'b11:   wave_out = tri_wave;
            default: wave_out = 16'sd0;
        endcase
    end

endmodule
